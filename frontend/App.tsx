...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (trimmed.length > 8 && (trimmed.startsWith("http://") || trimmed.startsWith("https://"))) {
      const handler = setTimeout(() => handleProbe(trimmed), 600);
  ...
  async function handleProbe(targetUrl: string) {
    ...
    try {
      ...
      setProbeError(err.message || "Invalid or unsupported video URL");
      setProbeData(null);
    } finally {
      setProbing(false);
    }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || "Failed to start download job");
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col justify-between py-12 px-4 selection:bg-emerald-500 selection:text-neutral-950">
      <div className="w-full max-w-xl mx-auto space-y-8">
        <header className="text-center space-y-2">
          ...
          <p className="text-sm text-neutral-400">Fast, lightweight multi-platform media downloader service</p>
        </header>
        ...
        <form onSubmit={submit} className="bg-neutral-900 rounded-2xl p-6 border border-neutral-800 shadow-xl space-y-5">
          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase tracking-wider text-neutral-400">
              Video URL (YouTube, Facebook, Instagram, etc.)
            </label>
            <div className="relative">
              <input
                ...
                placeholder="Paste video link here (e.g. YouTube, FB, IG)..."
                required
          ...
          {probeData && (
            <div className="bg-neutral-950/80 rounded-xl p-4 border border-neutral-800/80 flex gap-4 items-center">
              ...
              <div className="min-w-0 flex-1 space-y-1">
                <h3 className="text-xs font-semibold text-neutral-200 line-clamp-1">{probeData.title}</h3>
                <p className="text-xs text-neutral-400 flex items-center gap-1.5">
                  <span>{probeData.channel}</span>
                  <span>•</span>
                  <span>
                    {probeData.is_playlist
                      ? `Playlist (${probeData.playlist_count} videos)`
              ...
              <div className="grid grid-cols-2 gap-1 bg-neutral-950 p-1 rounded-xl border border-neutral-800">
                <button
                  ...
                  className={`flex items-center justify-center gap-1.5 py-2 text-xs font-medium rounded-lg transition-all ${
                    format === "mp4" ? "bg-neutral-800 text-emerald-400 shadow-sm" : "text-neutral-400 hover:text-neutral-200"
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1.5 py-2 text-xs font-medium rounded-lg transition-all ${
                    format === "mp3" ? "bg-neutral-800 text-emerald-400 shadow-sm" : "text-neutral-400 hover:text-neutral-200"
                  }`}
                >
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {Object.values(jobs).map((job) => (
                <div key={job.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 shadow-md space-y-3 relative group">
